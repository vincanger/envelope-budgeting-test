...
import { type UserBudgetProfile, type Invitation } from 'wasp/entities';
import { type User } from 'wasp/entities'; // For UserInfo type
...
export function MembersPage() {
  ...
  const { data: pendingInvites, isLoading: isLoadingInvites, error: errorInvites } = useQuery(
      getPendingInvitations, 
  ...
  return (
    <div className="p-4 w-full max-w-4xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
         ...
         {/* Show Invite Button only if authorized */}
         {canManageMembers && (
...
Enter the email address of the user you want to invite and select their role.
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsInviteDialogOpen(false)} disabled={isInviting}>Cancel</Button>
          ...
          <Table>
            ...
            <TableBody>
              {membersDisplayList.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={canManageMembers ? 3 : 2} className="text-center">No members found (except you).</TableCell>
                </TableRow>
              ) : (
                membersDisplayList.map((member) => {
                  ...
                  return (
                    <TableRow key={member.userId}>
                      <TableCell className="font-medium">{member.email}{isCurrentUser ? " (You)" : ""}</TableCell>
                      <TableCell><Badge variant={member.role === 'OWNER' ? 'default' : member.role === 'ADMIN' ? 'secondary' : 'outline'}>{member.role}</Badge></TableCell>
              ...
              <Table>
                ...
                <TableBody>
                  {!pendingInvites || pendingInvites.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center">No pending invitations.</TableCell>
                    </TableRow>
                  ) : (
                    pendingInvites.map((invite) => (
                      <TableRow key={invite.id}>
                        <TableCell className="font-medium flex items-center"><Mail className="mr-2 h-4 w-4 text-muted-foreground"/>{invite.email}</TableCell>
                        <TableCell><Badge variant={invite.role === 'ADMIN' ? 'secondary' : 'outline'}>{invite.role}</Badge></TableCell>
                        <TableCell className="text-sm text-muted-foreground"><Clock className="inline mr-1 h-3 w-3"/>{new Date(invite.expiresAt).toLocaleDateString()}</TableCell>
                        <TableCell className="text-right">
                          ...
                          >
                            {isRevokingId === invite.id ? 'Revoking...' : <><XCircle className="mr-1 h-4 w-4"/> Revoke</>}
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))
      ...
      <AlertDialog open={isRemoveConfirmOpen} onOpenChange={setIsRemoveConfirmOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setMemberToRemove(null)} disabled={!!isUpdatingRoleId}>Cancel</AlertDialogCancel>
            <AlertDialogAction 
              onClick={handleConfirmRemove} 
              disabled={!!isUpdatingRoleId}
