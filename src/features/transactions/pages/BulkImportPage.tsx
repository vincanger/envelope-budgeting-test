...
import { AlertCircle, CheckCircle2, Search } from 'lucide-react'; // Icons for status
...
type TransactionImportDataClient = {
  ...
  envelopeId?: number | null; // Add optional envelopeId
};
...
export function BulkImportPage() {
  ...
  // Add state for optional envelope assignment
  const [selectedEnvelopeId, setSelectedEnvelopeId] = useState<string>(''); // Empty string means no envelope selected
  ...
  const parseDateString = useCallback(
    ...
    [dateFormat]
  ); // Depends on the selected dateFormat

  // Function to validate a single row after parsing/editing
  const validateRow = useCallback(
    (row: Pick<ParsedRowData, 'Date' | 'Description' | 'Amount'>): string | undefined => {
    ...
    [numberFormat, dateFormat, parseDateString]
  ); // Add dateFormat and parseDateString to dependencies
  ...
  const resetState = () => {
    ...
    setSelectedEnvelopeId(''); // Reset selected envelope
  };
  ...
  const handleRowSelectionChange = (id: number, checked: boolean) => {
    setParsedData((prevData) => prevData.map((row) => (row.id === id ? { ...row, isSelected: checked } : row)));
  };
  ...
  const handleImport = async () => {
    if (validSelectedRows.length === 0) {
      setImportResult({
        ...
        errors: ['No valid transactions selected.'],
      });
      return;
    }
    setIsImporting(true);
    ...
    // Get the numeric envelope ID if selected
    const assignedEnvelopeId = selectedEnvelopeId ? parseInt(selectedEnvelopeId, 10) : null;
    if (selectedEnvelopeId && isNaN(assignedEnvelopeId as number)) {
      setImportResult({ successCount: 0, errorCount: selectedRows.length, errors: ['Invalid Envelope selected for bulk assignment.'] });
    ...
    validSelectedRows.forEach((row) => {
      ...
      transactionsToImport.push({
        ...
        envelopeId: assignedEnvelopeId, // Assign the selected envelope ID (or null)
      });
    });
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-6 border-b pb-6'>
          <div>
            ...
            <Input
              ...
              className='block w-full text-sm text-gray-500 file:mr-4 file:px-4 file:rounded-md file:border file:border-gray-300 file:text-sm file:font-semibold file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100 disabled:opacity-50'
          ...
          {/* Optional Envelope Assignment */}
          <div>
            <Label htmlFor='assignEnvelope' className='block text-sm font-medium text-gray-700 mb-1'>
              Assign All To Envelope (Optional)
            </Label>
            <Select value={selectedEnvelopeId} onValueChange={setSelectedEnvelopeId} disabled={isImporting || isLoadingEnvelopes}>
              <SelectTrigger id='assignEnvelope' className='w-full'>
                <SelectValue placeholder="Don't assign / Assign later" />
              </SelectTrigger>
              <SelectContent>
                {isLoadingEnvelopes && (
            ...
            <p className='mt-1 text-xs text-gray-500'>If selected, all imported transactions will be assigned to this envelope.</p>
          </div>
        </div>
          ...
          <div className='mt-6 p-4 border rounded-lg bg-gray-50 space-y-4'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
              {(['Date', 'Description', 'Amount'] as const).map((field) => (
                <div key={field}>
                  <Label htmlFor={`map-${field}`} className='block text-sm font-medium text-gray-700 mb-1'>
                    {' '}
                    {field} <span className='text-red-500'>*</span>{' '}
                  </Label>
                  <Select value={headerMappings[field]} onValueChange={(value) => handleMappingChange(field, value)}>
                    <SelectTrigger id={`map-${field}`} className='w-full bg-white'>
                      {' '}
        ...
        {showPreview && parsedData.length > 0 && (
          <div className='mt-6 space-y-4'>
                ...
                <TableBody>
                  {parsedData.map((row) => (
                    <TableRow key={row.id} data-state={row.isSelected ? 'selected' : ''} className={cn(row.validationError ? 'bg-red-100 hover:bg-red-200' : 'hover:bg-gray-50')}>
                      <TableCell>
                        {' '}
                        <Checkbox checked={row.isSelected} onCheckedChange={(checked) => handleRowSelectionChange(row.id, !!checked)} aria-label={`Select row ${row.originalIndex}`} />{' '}
                      </TableCell>
                      <TableCell>
                        {' '}
                        <Input type='text' value={row.Date} onChange={(e) => handleCellEdit(row.id, 'Date', e.target.value)} className='h-8 text-sm' placeholder='YYYY-MM-DD' />{' '}
                      </TableCell>
                      <TableCell>
                        {' '}
                        <Input value={row.Description} onChange={(e) => handleCellEdit(row.id, 'Description', e.target.value)} className='h-8 text-sm' />{' '}
                      </TableCell>
                      <TableCell className='text-right'>
                        <Input type='text' value={row.Amount} onChange={(e) => handleCellEdit(row.id, 'Amount', e.target.value)} className='h-8 text-sm text-right' placeholder='e.g. 1234.56 or 1234,56' />
                      </TableCell>
                      <TableCell className='text-xs'>
                        {row.validationError ? (
                          <span className='flex items-center text-red-600'>
                            <AlertCircle className='mr-1 h-4 w-4' /> {row.validationError}
                          </span>
                        ) : (
            ...
            <p className='text-sm text-gray-600'>
              {' '}
              Selected: {selectedRows.length} row(s). Valid for import: {validSelectedRows.length} row(s).{' '}
            </p>
            {/* Import Button - now appears after preview */}
            <Button onClick={handleImport} disabled={!canSubmit} className='mt-4' title={!canSubmit ? 'Select valid rows to import' : 'Import selected transactions'}>
